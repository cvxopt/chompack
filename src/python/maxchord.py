...
def maxchord(A, ve=None):
    ...
    Am = spmatrix(
        [d for d in chain.from_iterable(V)],
        [i for i in chain.from_iterable(E)],
        [i for i in chain.from_iterable([len(Ej) * [j] for j, Ej in enumerate(E)])],
        (n, n),
    )
