    ...
    def dot(X, Y):
        ...
        for k in range(X.symb.Nsn):
            ...
            for j in range(nk):
                ...
                val += 2.0*blas.dot(X.blkval, Y.blkval, offsetx=offset+ck*j+j,
                                    offsety=offset+ck*j+j, n=ck-j)
        return val
...
def syr2(X, u, v, alpha=1.0, beta=1.0, reordered=False):
    ...
    for k in range(symb.Nsn):
        ...
        blas.ger(uk, vk, blkval, m=na, n=nn,
                 offsetx=nn, offsetA=blkptr[k]+nn, ldA=nj, alpha=alpha)
        blas.ger(vk, uk, blkval, m=na, n=nn,
                 offsetx=nn, offsetA=blkptr[k]+nn, ldA=nj, alpha=alpha)
