from cvxopt import matrix, spmatrix, blas, printing
    ...
    for k in snpost:
        if snpar_[k] != k:
            ...
            if merge_function and merge_function(colp, colk, np, nk):
                ...
                snlist[snpar_[k]] = matrix(
                    sorted(list(snlist[k]) + list(snlist[snpar_[k]]))
                )
                snlist[k] = None
...
def embed(A, colcount, snode, snptr, snpar, snpost):
    ...
    PURPOSE
    Computes rowindices and column pointer for representative vertices
    in supernodes.
...
def relative_idx(colptr, rowidx, snptr, snpar):
    ...
    for k in range(len(snpar)):
        ...
        if p != -1:
            ...
            relidx[relptr[k]:relptr[k + 1]] = lfind(
                rowidx[colptr[k] + nk:colptr[k + 1]],
                rowidx[colptr[p]:colptr[p + 1]],
            )

    return relptr, relidx[:relptr[-1]]
...
def peo(A, p):
    ...
    for k in range(n):
        ...
        # indices of neighbors that correspond to strictly lower triangular
        # elements in reordered pattern
        r = set([rj for rj in ri[cp[v]:cp[v + 1]] if ip[rj] > k])

        for rj in r:
            if not r.issubset(set(ri[cp[rj]:cp[rj + 1]])):
                return False
...
class symbolic(object):
    ...
    def __init__(self, A, p=None, merge_function=None, **kwargs):
        ...
        assert (
            len([i for i, j in zip(Ap.I, Ap.J) if i == j]) == A.size[0]
        ), "the sparsity pattern of A must include diagonal elements"
        ...
        if p is not None:
            ...
            assert (
                len(p) == A.size[0]
            ), "length of permutation vector must be equal to the order of A"
            Ap = perm(Ap, p)
        ...
        pp = matrix(
            [snode[snptr[snpost[k]]:snptr[snpost[k] + 1]] for k in range(len(snpar))]
        )
    ...
    def __repr__(self):
        return "<symbolic factorization, n=%i, nnz=%i, nsn=%i>" % (
            self.n,
            self.nnz,
            self.Nsn,
        )
    ...
    def __str__(self):
        ...
        printing.options = {
            "iformat": "%1i",
            "dformat": "%1.0f",
            "width": printing.options["width"],
            "height": printing.options["height"],
        }
    ...
    def cliques(self, reordered=True):
        ...
        if reordered:
            return [
                list(self.snrowidx[self.sncolptr[k]:self.sncolptr[k + 1]])
                for k in range(self.Nsn)
            ]
        else:
            return [
                list(self.__p[self.snrowidx[self.sncolptr[k]:self.sncolptr[k + 1]]])
                for k in range(self.Nsn)
            ]
            ...
            return [
                list(
                    self.snrowidx[
                        self.sncolptr[k]
                        + self.snptr[k + 1]
                        - self.snptr[k]:self.sncolptr[k + 1]
                    ]
                )
                for k in range(self.Nsn)
            ]
    ...
    def supernodes(self, reordered=True):
        ...
        if reordered:
            return [
                list(self.snode[self.snptr[k]:self.snptr[k + 1]])
                for k in range(self.Nsn)
            ]
        else:
            return [
                list(self.__p[self.snode[self.snptr[k]:self.snptr[k + 1]]])
                for k in range(self.Nsn)
            ]
...
class cspmatrix(object):
    ...
    def __init__(self, symb, blkval=None, factor=False):
        ...
        if blkval is None:
            ...
            assert (
                len(blkval) == symb.blkptr[-1]
            ), "dimension mismatch: length of blkval is incorrect"
            self.blkval = blkval
    ...
    def spmatrix(self, reordered=True, symmetric=False):
        ...
        for k in range(self.symb.Nsn):
            ...
            for i in range(nn):
                j = snode[snptr[k] + i]
                blas.copy(
                    blkval, val, offsetx=blkptr[k] + nj * i + i, offsety=cp[j], n=nj - i
                )
    ...
    def diag(self, reordered=True):
        ...
        for k in range(self.symb.Nsn):
            ...
            for i in range(nn):
                D[snode[snptr[k] + i]] = self.blkval[blkptr[k] + i * w + i]
        if reordered:
            return D
        else:
            return D[self.symb.ip]
    ...
    def _iadd_spmatrix(self, X, alpha=1.0):
        ...
        for k in range(self.symb.Nsn):
            ...
            for i in range(nn):
                ...
                Il = [offset + r.index(idx) for idx in ri[cp[j]:cp[j + 1]]]

                blkval[Il] += alpha * val[cp[j]:cp[j + 1]]
        return
    ...
    def add_projection(self, A, alpha=1.0, beta=1.0, reordered=False):
        ...
        for k in range(self.symb.Nsn):
            ...
            blkval[blkptr[k]:blkptr[k + 1]] = beta * blkval[
                blkptr[k]:blkptr[k + 1]
            ] + alpha * (
                A[
                    snrowidx[sncolptr[k]:sncolptr[k + 1]],
                    snode[snptr[k]:snptr[k + 1]],
                ][:]
            )
