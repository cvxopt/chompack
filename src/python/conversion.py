from cvxopt import matrix, spmatrix, sparse, amd
import itertools
from chompack.symbolic import symbolic
from chompack.misc import tril, symmetrize
...
def symb_to_block(symb, coupling="full"):
    ...
    for i in range(Ncliques):
        offsets.append(offsets[-1] + dims[i] ** 2)
    ...
    for k in range(Ncliques):
        ...
        for j in range(nk):
            for i in range(j, wk):
                if i == j:
                    ...
                    sparse_to_block[nodes[j] * n + rows[i]] = (
                        offsets[k] + j * wk + i,
                        offsets[k] + i * wk + j,
                    )
        ...
        if symb.snpar[k] == k:
            continue  # skip if supernode k is a root supernode
        p = symb.snpar[k]
...
def convert_block(G, h, dim, **kwargs):
    ...
    :param dim:               integer
    :param merge_function:    merge heuristic (optional)
    :param coupling:          mode of conversion (optional)
    :param max_density:       float (default: 0.4)

    The following example illustrates how to apply the conversion method
    to a one-block SDP:
    ...
    in the aggregate sparsity pattern is less than or equal to
    `max_density*dim`.
    ...
    if coupling == "sparse":
        ...
        coupling += spmatrix(
            1.0,
            [i for j in range(Va.size[0]) for i in range(j, min(Va.size[0], j+2))],
            [j for j in range(Va.size[0]) for i in range(j, min(Va.size[0], j+2))],
            Va.size,
        )
        ...
        coupling = spmatrix(
            ...
            [
                i
                for j in range(Va.size[0])
                for i in range(j, min(Va.size[0], j + bw + 1))
            ],
            [
                j
                for j in range(Va.size[0])
    ...
    for j in range(m):
        ...
        for i in range(cp[j + 1] - cp[j]):
            ...
            col = ri[cp[j] + i] // dim
            if row < col:
                continue  # ignore upper triangular entries
            k1 = ip[row]
            k2 = ip[col]
    ...
    for j in range(len(constraints)):
        ...
        if len(constraints[j]) == 2:
            ...
            iv = sorted(
                [(i1, 1.0), (i2, 1.0), (j1, -1.0), (j2, -1.0)], key=lambda x: x[0]
            )
            jl = 4 * [ncon]
            ncon += 1
            if tc == "z":
    ...
    idx, blki = zip(*idx)
    blki = matrix(blki)
    Il = [v % dim for v in idx]
    Jl = [v // dim for v in idx]
    n = sum([di**2 for di in dims])

    return (G_converted, h_converted, G_coupling, dims), (blki, Il, Jl, n), F


def convert_conelp(c, G, h, dims, A=None, b=None, **kwargs):
    """
    Applies the clique conversion method of Fukuda et al. to the positive
    semidefinite blocks of a cone LP.
    ...
    G2 = spmatrix(
        [v for v in itertools.chain(*V)],
        [v for v in itertools.chain(*I)],
        [v for v in itertools.chain(*J)],
        tuple(offset),
    )
