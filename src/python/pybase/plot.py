...
if __mpl__ and __np__ and __scipy__:
    ...
    def spy(symb, reordered=True, symmetric=True, **kwargs):
        ...
        M = scipy_sparse.csc_matrix(
            (np.array(list(val)), np.array(list(rowidx)), np.array(list(colptr)))
        )
...
if __nx__ and __mpl__:
    ...
    def sparsity_graph(symb, **kwargs):
        ...
        for k in range(symb.Nsn):
            ...
            G.add_edges_from(
                [(ck[i], ck[j]) for i in range(len(ck)) for j in range(i, len(ck))]
            )
        if layout is None:
...
if __nx__ and __mpl__:
    ...
    def etree_graph(symb, **kwargs):
        ...
        G.add_edges_from(
            [(symb.snpar[k], k) for k in range(symb.Nsn) if symb.snpar[k] != k]
        )
        if layout is None:
