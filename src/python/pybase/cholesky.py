...
def cholesky(X):
    ...
    for k in snpost:
        ...
        if na > 0:
            ...
            blas.trsm(
                F, F, m=na, n=nn, ldA=nj, ldB=nj, offsetB=nn, transA="T", side="R"
            )

            # compute Uk = Uk - L_{Ak,Nk}*inv(D_{Nk,Nk})*L_{Ak,Nk}'
            if nn == 1:
                blas.syr(
                    F, F, n=na, offsetx=nn, offsetA=nn * nj + nn, ldA=nj, alpha=-1.0
                )
            else:
                blas.syrk(
                    F,
            ...
            blas.trsm(F, F, m=na, n=nn, ldA=nj, ldB=nj, offsetB=nn, side="R")
